...
% right before document start
%% \usepackage{embedall} % should embed fonts and also the latex code so I cannot lose it.
% but since this is not working with my image for some reason... a diffferent way
% https://tex.stackexchange.com/questions/13187/is-there-some-way-to-embed-latex-source-code-in-a-pdf-file
%\usepackage{embedfile}
%\embedfile{./CodeBlocks.tex}
% Above worked, but I'd rather be able to also link to embedded files, so...
% https://tex.stackexchange.com/questions/19025/embed-link-to-an-embedded-file
\usepackage{navigator}
\embeddedfile{mysourcecode}{./day8.tex}
% see later in the file for a link of the form
% \openfilelink{myfile.pdf}{Link to the embedded file \texttt{myfile.pdf}}
% and to color the link differently:
% \usepackage{hypgotoe}
% \href{gotoe:embedded=file.pdf}{description}
% but either only seem to work with pdfs
%
% It's also possible to store a password-encrypted file: https://tex.stackexchange.com/questions/26067/is-it-possible-to-embed-attach-a-file-password-protected-in-a-pdf
%
% Alternatively:
% \usepackage{attachfile2}
% \textattachfile[color= 0 0 0]{Aufgabenstellung_EricMink.pdf}{filetext}
% -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
...
%\newcounter{layercounter} %counter already exists
...
\loop
  ...
  % for each pixel in the first layer, search all layers unti a pixel is opaque
	...
	{\loop
	  \addtocounter{layercounter}{1}
		\def\totalpos{\numexpr \value{charcounter} + ( \value{layercounter} * \layersize )}
		\StrChar{\workingstr}{\totalpos}[\currentchar]
		\setcounter{pixelvalue}{\inteval{\currentchar}}
		\typeout{Considering Layer \thelayercounter ~for pixel \thecharcounter}
		...
		\ifnum \value{layercounter}=\inteval{\numlayers -1}
	...
	\ifnum \value{pixelvalue}=\inteval{0 - 1}
		\setcounter{pixelvalue}{2}
	\fi
	\typeout{Found \thepixelvalue ~in Layer \thelayercounter ~for pixel \thecharcounter}
...
I should probably also put the shellescape stuff into an optional branch. $\cap$.

\section{Performance}
PDF Elapsed Time: \showthe\pdfelapsedtime
\begin{myquote}\begin{quote}
The value of \pdfelapsedtime is the number of seconds elapsed multiplied by 65536. \citelist{\cite{robertson}}
