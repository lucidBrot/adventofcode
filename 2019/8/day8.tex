...
Test: \StrLen{\fileline}\newline
\StrLen{\fileline}[\layerafterthelast]%find the strlen and store it in \MyStrLen
% see https://tex.stackexchange.com/a/15424/102826

The input file contains \layerafterthelast minus one characters. % TODO: compute the index where to stop by adding one.
...
\loop
	\loop
	  ...
	  \ifx 0\currentchar
	    ...
	    Advanced currentlayerzerocount to \currentlayerzerocount
	...
	Layer \the\currentlayer has more zeros than the current best layer (\the\bestlayer ) so we skip ahead to character at index \digictr to start the next layer.
	    \fi
	  \fi
	\ifnum \digitctr<\inteval{\layersize}
	\repeat
...
\ifnum \currentlayer<\layerafterthelast
\repeat
