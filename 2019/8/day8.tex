...
% flag condition to debug the loop by rewriting it piece by piece
\newif\ifoldcomputationenabled
\oldcomputationenabledfalse
\section{ Execution Two}
...
\end{quote}\end{myquote}
An alternative to using globals would be to use tikz loops with the \texttt{remember} option \citelist{\cite{tikzremember}}.

\newcount\outerloopcount
\outerloopcount=0
\loop
  {
    	\advance\outerloopcount by1
           \the\outerloopcount,
	\global\outerloopcount = \outerloopcount
           \ifnum \the\outerloopcount<3
           \repeat
  }
\\outerloopcount: \the\outerloopcount

% Aand now repeat the inner loop below once per layer. 
\ifoldcomputationenabled
\section{Old Computation}
