...
\edef\tmppixelvalue{\thepixelvalue}
	\myappendstring{\immediate\tmppixelvalue}
\thepixelvalue
...
Let me just perform some magic - Consider \autoref{fig:magic} \citelist{\cite{collectvar}}. There's only one problem with that: It assumes that  the input strings are constant. Since I input the value of a counter, mine are not, and the magical string that ends up being collected is just a repetition of the last value it had. Adding \texttt{\textbackslash immediate} and \texttt{\textbackslash edef} was not helpful.
