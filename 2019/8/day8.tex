...
During Part 2, I'm revisiting this section again, though. because \code{StrMid} is just so very slow! Now I could of course hardcode a command that returns the correct character based on the argument, but isn't that a bit too cheaty? Maybe I'll fall back to that.
\immediate\write18{ls xyz.* > flerimmcblerim.dat}
  ...
  % for each pixel in the first layer, search all layers unti a pixel is opaque
	...
	{\loop
	  \addtocounter{layercounterx}{1}
		...
		%\StrMid{\workingstr}{12}{12}[\currentchar]
